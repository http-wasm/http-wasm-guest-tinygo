...
// tryCompileGotipExample compiles with gotip, which is used until Go 1.21 is
// out, as before that there is no GOOS=wasip1.
func tryCompileGotipExample(name string) (wasm []byte) {
	...
	} else if wasm, err = compileWasip1Wasm(gotipBin, path.Join("..", "..", "examples", name)); err != nil {
...
func compileWasip1Wasm(gotipBin string, dir string) ([]byte, error) {
